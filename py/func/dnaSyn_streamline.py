        ...
        #qsub mode
            ...
            for n_cmd,cmd in enumerate(self.settings.pipeline):
                ...
                if cmd=="import":
                    ...
                    if is_qc:
                        file_endfix="_srcSeq.QC.tsv.gz"
                        file_prefix=[os.path.basename(i).replace(file_endfix,"") for i in glob.glob(self.settings.outdir+"/qc/*") if re.search(file_endfix,i)]
                    else:
                        file_endfix="_srcSeq.tsv.gz"
                        file_prefix=[os.path.basename(i.replace(file_endfix,"")) for i in glob.glob(self.settings.outdir+"/import/*") if re.search(file_endfix,i)]
