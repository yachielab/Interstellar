...
def run(sampledir_list,cfg_raw,qcfg,is_qsub,is_multisample,param_dict,proj_dir):
    used_commands=[]
    cfg=settingImporter.config_extract_value_ext(cfg_raw)
    ...
    for sampledir in sampledir_list:
        ...
        if is_qsub:
            ...
            print(qcmd_base)
            print("\n###")
            print(param_dict[os.path.basename(sampledir)]["target_prefix_list"])

            #collect input files
            file_pool=[]
            all_files=glob.glob(sampledir+"/filesplit/*/*")
            print(param_dict[os.path.basename(sampledir)])
