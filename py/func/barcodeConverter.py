...
def build_count_tree_parallel_wrapper(df_chunk,roots,edge_dict,d2s_dict,settings,globalComponents,sample_now,count_tree,ncore):
    ...
    count_tree_list = Parallel(n_jobs=ncore,require='sharedmem',verbose=10)(
...
def optimize_value_table_parallel_wrapper(df_chunk,d2s_dict,roots,edge_dict,settings,globalComponents,sample_now,global_used_in_dest,destname_dict,tree,ncore):
    ...
    retLst = Parallel(n_jobs=ncore,require='sharedmem',verbose=10)(
...
def demultiplex_tsv_parallel_wrapper(s_seq_chunk,demulti_key,key_series,settings,ncore):
    ...
    key_iden_list_now = Parallel(n_jobs=ncore,require='sharedmem',verbose=3)(
...
def concat_demultiplex_files_parallel_wrapper(key_set,sampledir,demulti_filename_list,ncore,format_type):
    if format_type=="tsv":
        Parallel(n_jobs=ncore,require='sharedmem',verbose=3)(delayed(tsv_concat_process)(key,sampledir,demulti_filename_list) for key in key_set)
    else:
        Parallel(n_jobs=ncore,require='sharedmem',verbose=3)(delayed(fastq_concat_process)(key,sampledir,demulti_filename_list) for key in key_set)
