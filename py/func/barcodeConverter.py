...
def dval_to_sval_relationship(convertOptions):
    dval_to_sval_relationship={}
    for component in convertOptions:
        opt_now=convertOptions[component]
        if opt_now["is_combination"]: #source value can have combination
            s_values=[opt_now[i] for i in opt_now if "src_corrected_components" in i]
        else:
            s_values=[opt_now["src_corrected_components"]]
        
        s_value="+".join(s_values)
        dval_to_sval_relationship[component]=s_value

    return dval_to_sval_relationship


def parse_constraint(convertOptions):
    globalComponents=[]
    edge_dict=dict(parent=[],child=[])
    for component in convertOptions:
        opt_now=convertOptions[component]
        if opt_now["type"]=="localConvert":
            edge_dict["child"].append(component)
            edge_dict["parent"].append(opt_now["constraint"])
        elif opt_now["type"]=="globalConvert":
            globalComponents.append(component)

    roots=list(set(edge_dict["parent"])-set(edge_dict["child"]))
    # tips =list(set(edge_dict["child"])-set(edge_dict["parent"]))
    globalComponents=list(set(globalComponents)-set(roots))
    
    return roots,edge_dict,globalComponents


def to_svalue_prime(s_val_chunk,dval_to_sval_relationship):
    #convert s-value into new s-value interpreting the combination

    for idx,component in enumerate(dval_to_sval_relationship):
        component_svalue=dval_to_sval_relationship[component]

        # Generate series of s-values 
        if "+" in component_svalue:
            component_svalue = component_svalue.split("+")
            series_svalue = s_val_chunk[component_svalue[0]].str.cat(s_val_chunk[component_svalue[1:]],sep="+")
        else:
            series_svalue = s_val_chunk[component_svalue]
        series_svalue.name=component
...
def buildConstraintTree_singleScan(constraintList,convertedDataframe,convertOptions,s_val_chunk,constraintTree):
    ...
    for cnt,i in enumerate(constraintList):
        ...
        if set(constraintGroup) <= set(convertedDataframe.columns):
            opt_now=convertOptions[localComponent]
...
def buildConstraintTree_v2(constraintList,convertedDataframe,convertOptions,s_val_chunk,constraintTree):
    processed=[]
    convertedDataframe=convertedDataframe.applymap(str)
    for cnt,i in enumerate(constraintList):
        localComponent=i[0]
...
def buildConstraintTree(constraintList,convertedDestination,convertOptions,correctedIndexDict):
...
def buildReference(opt_now,info_dic,max_default=6000000): #12bp程度まで
    if opt_now["type"]=="whitelistExport":
        with open(opt_now["whitelist"],mode="rt",encoding="utf-8") as f:
            reference_now=[regex.sub("\n","",i) for i in f]
            random.seed(0)
            random.shuffle(reference_now)
    elif opt_now["type"]=="randomExport":
        d_val=opt_now["d_val"]
        maxsize=info_dic[d_val]
        if maxsize < max_default:
            maxsize=max_default

        seqlength=opt_now["length"]
...
def getConvSeq(query,reference):
    query_now=query
    if query_now >= len(reference) or query_now < 0:
        return None
    else:
        return reference[query_now]
